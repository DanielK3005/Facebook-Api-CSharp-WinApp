...
namespace BasicFacebookFeatures
{
    ...
    {
        private FormCheckersMenu m_CheckersMenu;
        ...
        public FormGameBoard(string i_playerName)
        {
            initializeComponent(i_playerName);
        }

        private void initializeComponent(string i_playerName)
        {
            int sizeOfBoard;

            m_CellMoveFrom = new Position();
            m_CellMoveTo = new Position();
            m_CheckersMenu = new FormCheckersMenu(i_playerName);

            if (ensurePlayersNames())
            {
                sizeOfBoard = m_CheckersMenu.SizeOfBoard / 2;

                m_ButtonsGameBoard = new Button[m_CheckersMenu.SizeOfBoard, m_CheckersMenu.SizeOfBoard];
                ...
                for (int i = 0; i < m_CheckersMenu.SizeOfBoard; i++)
                {
                    for (int j = 0; j < m_CheckersMenu.SizeOfBoard; j++)
                    {
                ...
                m_Gameplay = new Gameplay(m_CheckersMenu.TextBoxPlayer1Name, m_CheckersMenu.TextBoxPlayer2Name, (uint)m_CheckersMenu.SizeOfBoard, m_CheckersMenu.IsPlayingAgainstFriend);
                m_Gameplay.OnMove += M_Gameplay_OnMove;
                labelFirstPlayerScore.Text = m_CheckersMenu.TextBoxPlayer1Name + ": " + m_Gameplay.FirstPlayerScore;
                ...
                labelSecondPlayerScore.Text = m_CheckersMenu.TextBoxPlayer2Name + ": " + m_Gameplay.SecondPlayerScore;
                ...
                this.m_PictureBoxWhichPlayerTurn.Image = Properties.Resources.O_removebg_preview__1_;
                ...
                this.m_PictureBoxFirstPlayer.Image = Properties.Resources.O_removebg_preview__1_;
                ...
                this.m_PictureBoxSecondPlayer.Image = Properties.Resources.x;
                ...
                this.ClientSize = new System.Drawing.Size(m_ButtonsGameBoard[m_CheckersMenu.SizeOfBoard - 1, m_CheckersMenu.SizeOfBoard - 1].Right + 20, m_ButtonsGameBoard[m_CheckersMenu.SizeOfBoard - 1, m_CheckersMenu.SizeOfBoard - 1].Bottom + 20);
                this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
                this.Text = "English Checkers";
                this.BackgroundImage = Properties.Resources.checkersLogo;
                this.BackgroundImageLayout = System.Windows.Forms.ImageLayout.Tile;
                this.MaximizeBox = false;
                this.FormBorderStyle = FormBorderStyle.Fixed3D;
                System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(FormCheckersMenu));
        ...
        private bool ensurePlayersNames()
        {
            ...
            {
                if (m_CheckersMenu.ShowDialog() == DialogResult.OK)
                {
                    if (m_CheckersMenu.ArePlayersNamesNotEmpty())
                    {
        ...
        private void putSymbolOnButton(int i_IndexRow, int i_IndexColoum)
        {
            if ((i_IndexRow + i_IndexColoum) % 2 != 0)
            {
                if (i_IndexRow < (m_CheckersMenu.SizeOfBoard / 2) - 1)
                {
                    //O
                    this.m_ButtonsGameBoard[i_IndexRow, i_IndexColoum].BackgroundImage = Properties.Resources.O_removebg_preview__1_;
                }
                else if (m_CheckersMenu.SizeOfBoard / 2 < i_IndexRow && i_IndexRow < m_CheckersMenu.SizeOfBoard)
                {
                    //X
                    this.m_ButtonsGameBoard[i_IndexRow, i_IndexColoum].BackgroundImage = Properties.Resources.x;
                }
                else
                {
        ...
        private void changePictureTurn()
        {
            ...
            {
                this.m_PictureBoxWhichPlayerTurn.Image = Properties.Resources.x;
            }
        }
        ...
        private Position getClickedPosition(Button i_ClickedButton)
        {
            ...
            clickedPosition.Row = (uint)(countPosition / m_CheckersMenu.SizeOfBoard);
            clickedPosition.Column = (uint)(countPosition % m_CheckersMenu.SizeOfBoard);
        ...
        private static Image getSymbol(Cell i_currentCell)
        {
            ...
            if (i_currentCell.IsActiveCell)
            {
                if (i_currentCell.cellState == CellState.eCellState.BelongToFirstPlayer)
                {
                    ...
                    {
                        symbol = Properties.Resources.goldCrown;
                    }
                    else
                    {
                        symbol = Properties.Resources.O_removebg_preview__1_;
                    }
                }
                ...
                if (i_currentCell.cellState == CellState.eCellState.BelongToSecondPlayer)
                {
                    if (i_currentCell.IsKing)
                    {
                        symbol = Properties.Resources.redCrown;
                    }
                    else
                    {
                        symbol = Properties.Resources.x;
                    }
                }
            }
        ...
        private void gameOverOperation()
        {
            ...
            // winnig message 
            DialogResult userAnswerAboutRestart = MessageBox.Show(string.Format(@"{0} Do you want to play another round?", winDeclaration), "Game Over", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void declareOnWinner(out string o_WinDeclaration)
        {
            ...
            {
                o_WinDeclaration = string.Format("{0} won this game!", m_CheckersMenu.TextBoxPlayer1Name);
            }
            else if (differenceResult < 0)
            {
                o_WinDeclaration = string.Format("{0} won this game!", m_CheckersMenu.TextBoxPlayer2Name);
            }
            else
            {
